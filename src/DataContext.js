...
export const DataProvider = ({ user, children }) => {
  ...
  const setNewPicks = (newRankedPicks, newRankedRanks, newFilePicks, newConfigId) => {
    ...
    const updatedPicks = {...fetchedCurPicks};
    ...
    setFetchedCurPicks(updatedPicks);
  }
  ...
  return (
    <DataContext.Provider value={{ fetchedCurWeek, fetchedPlayers, fetchedSubmissions, fetchedGameResults, fetchedMatchupsResponse, fetchedCurPicks, setNewPicks, fetchedRankPicks, fetchedRankedRanks, fetchedFilePicks }}>
      {children}
